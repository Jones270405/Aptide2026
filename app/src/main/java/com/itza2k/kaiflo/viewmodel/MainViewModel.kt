...
class MainViewModel(application: Application, private val speak: Boolean) : AndroidViewModel(application) {
    ...
    private val _ttsRate = MutableStateFlow(
        (sharedPreferences.getFloat("tts_rate", 1.0f)).coerceIn(0.5f, 2.0f)
    )
    ...
    fun updateTtsRate(rate: Float) {
        ...
        _ttsRate.value = r
        sharedPreferences.edit().putFloat("tts_rate", r).apply()
    ...
    private fun loadGroup(){
        viewModelScope.launch {
            val groups = repository.loadAllGroups().toList()
            val pinned = loadPinnedGroups()
            _groupList.value = groups.sortedWith(compareByDescending<Group> { pinned.contains(it.groupId) }
    ...
    // ---- Pin groups (conversations) ----
    private fun loadPinnedGroups(): Set<Long> {
        val raw = sharedPreferences.getString("pinned_groups", "") ?: ""
        return raw.split(',').filter { it.isNotBlank() }.mapNotNull { it.toLongOrNull() }.toSet()
    }

    private fun savePinnedGroups(set: Set<Long>) {
        val raw = set.joinToString(",")
        sharedPreferences.edit().putString("pinned_groups", raw).apply()
    }

    fun isGroupPinned(groupId: Long): Boolean = loadPinnedGroups().contains(groupId)

    fun togglePinGroup(groupId: Long) {
        val current = loadPinnedGroups().toMutableSet()
        if (current.contains(groupId)) current.remove(groupId) else current.add(groupId)
        savePinnedGroups(current)
        // resort list
        viewModelScope.launch {
            val groups = repository.loadAllGroups().toList()
            _groupList.value = groups.sortedWith(compareByDescending<Group> { current.contains(it.groupId) }
                .thenByDescending { it.groupId })
        }
    }
    ...
    fun buildCurrentChatMarkdown(): String {
        val sb = StringBuilder()
        chatList.forEach { msg ->
            val who = if (msg.isMe) "You" else "Assistant"
            val text = msg.englishText.ifBlank { msg.translatedText }
            sb.append("**$who:**\n")
            sb.append(text)
            sb.append("\n\n")
        }
    ...
    fun shareText(context: Context, text: String, title: String = "Share chat") {
...
//        }
    ...
    private fun setAlarmFromPrompt(dayMatch: String?, timeMatch: MatchResult?, relativeTimeMatch: MatchResult?) {
        ...
        if (relativeTimeMatch != null) {
            ...
            val amPmRaw = timeMatch.groupValues.getOrNull(4) ?: ""
            val amPm = amPmRaw.uppercase(Locale.ROOT)

            // Convert time to 24-hour format if AM/PM is specified; otherwise infer next occurrence
            var hour24 = when {
                amPm == "PM" && hour != 12 -> hour + 12
                amPm == "AM" && hour == 12 -> 0
                amPm.isEmpty() -> hour // tentatively treat as 24h if user spoke "17:30"; will adjust below
                else -> hour
            }
            ...
            // If AM/PM was not specified and computed time is in the past for today, move to next day
            if (amPm.isEmpty()) {
        ...
        val intent = Intent(AlarmClock.ACTION_SET_ALARM).apply {
            ...
            // Showing the alarm UI improves compatibility across OEMs (SKIP_UI may be ignored without privileged permission)
    ...
    private fun setReminderFromPrompt(dayMatch: String?, timeMatch: MatchResult?, relativeTimeMatch: MatchResult?,context: String) {
        ...
        val intent = Intent(AlarmClock.ACTION_SET_ALARM).apply {
            ...
            putExtra(AlarmClock.EXTRA_HOUR, calendar.get(Calendar.HOUR_OF_DAY))
            putExtra(AlarmClock.EXTRA_MINUTES, calendar.get(Calendar.MINUTE))
            // Showing the system Alarm UI improves reliability across OEMs
            putExtra(AlarmClock.EXTRA_SKIP_UI, false)
    ...
    fun loadYoutubeKey(): String? {
        ...
        // If the key is not in SharedPreferences, return null and let UI prompt user via Settings
        if (youtubeKey.isNullOrBlank()) {
            Log.d("MainViewModel", "YouTube key not found in SharedPreferences.")
        }
    ...
    fun loadChatKey(): String? {
        ...
        if (chatKey.isNullOrBlank()) {
            Log.d("MainViewModel", "Chat key not found in SharedPreferences.")
        }
